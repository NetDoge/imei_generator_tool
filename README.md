...
生成 IMEI：每次生成 IMEI 時，程序會隨機選擇一個已匯入的前綴，然後生成剩餘的 6 位隨機數字，最後通過 Luhn 算法計算第 15 位校驗碼（8 位前綴 + 6 位隨機 + 1 位校驗）。

CSV 匯入：分隔符支持逗號（`,`）或分號（`;`），前綴必須為 8 位純數字，型號可含空格。格式錯誤或前綴不合法的行會自動跳過，並在匯入完成時顯示成功/跳過數量。
